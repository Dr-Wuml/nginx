...
#OBJS = nginx.o ngx_conf.o 一个个太麻烦，可选择下面的方法
OBJS = $(SRCS:.cpp=.o)
...
#定义存放obj文件的目录，目录统一到一个位置后才方便后续的链接，不然到各个子目录，不好链接
#注意下面这个字符串，末尾不要有空格等，否则会有语法错误
...
#----------##############################################################################--------------#
#make找第一个目标开始执行[每个目标[要生成的东西],其实都是定义一种依赖关系]，目标格式为：
#目标:目标依赖【可以省略】
#   要执行的命令【可以省略】
#如下时开始执行的入口，执行就找到依赖$(BIN)去执行，同时，这里也依赖了$(DEPS),这样可以生成很多.d文件
all:$(DEPS) $(OBJS) $(BIN)
